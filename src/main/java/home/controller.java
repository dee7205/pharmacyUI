...
//            itemsSoldTable.setItems(sortedItemsSoldData);
    ...
    public void prepareTransactionWindowComboBox(){
        ...
        transactionWindow_comboBox.setOnAction(event ->{
            ...
            if (selectedItem != null && !selectedItem.isEmpty()){
                ...
                if (item != null)
                    transactionWindow_currentQty_textfield.setText(Integer.toString(item.getQuantity()));
                else
    ...
    public void addTransactionWindow(){
        ...
        Item item = new SQL_DataHandler().getItem(itemName);
        ...
        for (TransactionWindow trans : transactionWindowList){
            if (trans.getItemName().equals(tw.getItemName()) && trans.getSellQty() + tw.getSellQty() <= item.getQuantity()){
                trans.setSellQty(trans.getSellQty() + tw.getSellQty());
                transactionWindowTable.setItems(initialTransactionWindowData());
                return;
            }
        }

        if (soldQtyInt + tw.getSellQty() <= item.getQuantity()){
